...
这样就不用担心多个窗口注册了同名的事件时，渲染进程发送该名称的事件到主进程，所有窗口对象都收到该事件通知。（Electron 22 无效，仍需全局唯一事件名）
...
## 3.6 Debug
由于本模板使用的是脚本启动，因此调试main process时不能调试源码

1. 使用外部调试工具。
  
> script: `"dev": "node scripts/dev-server.js development --inspect=9229"`

> 浏览器地址栏中输入 `chrome://inspect`

> 然后点击 `Open dedicated DevTools for Node`

2. vscode
   
> script: `"dev": "node inspect scripts/dev-server.js development",`
