...
class SlientPrintWindow extends WindowBase{
  constructor(){
    ...
    this.openRouter("/index1");
  }

  protected registerIpcMainHandler(): void{ 
    this._browserWindow?.webContents.on("did-stop-loading", (event) => {
      this._browserWindow?.webContents.executeJavaScript(`
      console.log('Hello from executeJavaScript!');
      window.slientWindowAPI.printSlientWindow();
      console.log('Hello from 已执行!1111');

    `);
    });

    ipcMain.on("print-slient-window", (event) => {
      this._browserWindow?.webContents.print({ 
        silent: true,
        deviceName: "Microsoft Print to PDF" });
    });
  }
}
export default SlientPrintWindow;