    ...
    session.defaultSession.webRequest.onHeadersReceived((details, callback) => {
      callback({
        responseHeaders: {
          ...
          "Content-Security-Policy": [ "default-src 'self'; frame-src https://www.baidu.com/; script-src 'self' 'unsafe-inline' 'unsafe-eval'; style-src 'self' 'unsafe-inline' http://localhost:8098 http://39.99.237.1:9091; img-src 'self' data:" ],
        },
      });
    });
  });
