<template>
  <div class="root">
    <div class="titlebar">
      <Titlebar title="Electron + Vue3 + Vite boilerplate" />
    </div>
    <div class="content">
      <div class="logo">
      ...
      <a-collapse v-model:activeKey="activeKey" class="collapse">
        <a-collapse-panel key="1" header="功能">
          <a-space wrap>
            ...
            <a-button @click="onCallNativeLibraryFnAdd">
              调用本地库方法add
            </a-button>
        ...
        <a-collapse-panel key="3" header="文件下载">
          ...
          >
            <a-form-item
              label="下载链接"
              name="url"
              :rules="[{ required: true, message: 'Please input file download url!' }]"
            >
      ...
      >
        <div class="exit-msg-title">
          <font-awesome-icon icon="fa-solid fa-triangle-exclamation" color="#ff0000" /> 警告
        </div>
...
async function onCallNativeLibraryFnAdd(){
  let result = await simpleDll.add(1, 2);
  message.success(result.value);
}

async function onGotoWebview(){
